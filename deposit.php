...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    $amount = $_POST['amount'];
if($amount<0){
    echo "<script>alert('Please enter the corerect amount !')</script>";
}else{
...
$update = "UPDATE users SET balance='$new_balance' WHERE username='$username'";

mysqli_query($conn, $update);
date_default_timezone_set('Asia/Kathmandu');   
$date = date("Y-m-d H:i:s");
$transaction = "INSERT INTO transactions(transaction_type, transaction_date, t_amount, username,Sender_acc,Receiver_acc)
                VALUES('Deposit', '$date', '$amount', '$username','$accountNum','$accountNum')";
...
<style>
    ...
    * {
    ...
    .roshan {
        background-color: rgba(12, 12, 12, 0.597);
        display: inline-block;
        padding: 40px;
        border-radius: 15px;
        margin-top: 23vh;
    ...
    .butt:hover {
        background-color: rgba(243, 197, 12, 0.83);
        color: black;
        transform: scale(1.03);
    }
    ...
    .input:hover {
        background-color: rgba(219, 210, 210, 0.808);
        transition: 0.5s;
        color: black;
    ...
    .container {
        background-image: url(wp13577592-ghibli-night-wallpapers.jpg);
